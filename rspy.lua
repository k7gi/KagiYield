...
if (getgenv().SimpleSpyExecuted or getgenv().KagiSpyExecuted) and type(getgenv().SimpleSpyShutdown or getgenv().KagiSpyShutdown) == "function" then
    if getgenv().SimpleSpyShutdown then getgenv().SimpleSpyShutdown() end; if getgenv().KagiSpyShutdown then getgenv().KagiSpyShutdown() end
end
...
function ErrorPrompt(Message,state)
    if getrenv then
        ...
        prompt:setErrorTitle("KagiSpy V3 Error")
...
local KagiSpy3 = Create("ScreenGui",{ResetOnSpawn = false})
local Storage = Create("Folder",{})
local Background = Create("Frame",{Parent = KagiSpy3,BackgroundColor3 = Color3.new(1, 1, 1),BackgroundTransparency = 1,Position = UDim2.new(0, 500, 0, 200),Size = UDim2.new(0, 450, 0, 268)})
local LeftPanel = Create("Frame",{Parent = Background,BackgroundColor3 = Color3.fromRGB(25, 25, 25),BorderSizePixel = 0,Position = UDim2.new(0, 0, 0, 19),Size = UDim2.new(0, 131, 0, 249)})
...
local RightPanel = Create("Frame",{Parent = Background,BackgroundColor3 = Color3.fromRGB(15, 15, 15),BorderSizePixel = 0,Position = UDim2.new(0, 131, 0, 19),Size = UDim2.new(0, 319, 0, 249)})
local CodeBox = Create("Frame",{Parent = RightPanel,BackgroundColor3 = Color3.fromRGB(10, 10, 10),BorderSizePixel = 0,Size = UDim2.new(0, 319, 0, 119)})
...
local TopBar = Create("Frame",{Parent = Background,BackgroundColor3 = Color3.fromRGB(15, 15, 15),BorderSizePixel = 0,Size = UDim2.new(0, 450, 0, 19)})
local Simple = Create("TextButton",{Parent = TopBar,BackgroundColor3 = Color3.new(1, 1, 1),AutoButtonColor = false,BackgroundTransparency = 1,Position = UDim2.new(0, 5, 0, 0),Size = UDim2.new(0, 57, 0, 18),Font = Enum.Font.SourceSansBold,Text = "KagiSpy",TextColor3 = Color3.fromRGB(253, 116, 255),TextSize = 14,TextXAlignment = Enum.TextXAlignment.Left})
...
local ToolTip = Create("Frame",{Parent = KagiSpy3,BackgroundColor3 = Color3.fromRGB(15, 15, 15),BackgroundTransparency = 0.1,BorderColor3 = Color3.fromRGB(253, 116, 255),Size = UDim2.new(0, 200, 0, 50),ZIndex = 3,Visible = false})
local TextLabel = Create("TextLabel",{Parent = ToolTip,BackgroundColor3 = Color3.new(1, 1, 1),BackgroundTransparency = 1,Position = UDim2.new(0, 2, 0, 2),Size = UDim2.new(0, 196, 0, 46),ZIndex = 3,Font = Enum.Font.SourceSans,Text = "This is some slightly longer text.",TextColor3 = Color3.fromRGB(253, 116, 255),TextSize = 14,TextWrapped = true,TextXAlignment = Enum.TextXAlignment.Left,TextYAlignment = Enum.TextYAlignment.Top})

-------------------------------------------------------------------------------

local selectedColor = Color3.fromRGB(253, 116, 255)
...
getgenv().KAGISPYCONFIG_MaxRemotes = 300
local indent = 4
local scheduled = {}
...
xpcall(function()
    if isfile and readfile and isfolder and makefolder then
        local cachedconfigs = isfile("KagiSpy//Settings.json") and jsond(readfile("KagiSpy//Settings.json"))
        ...
        if not isfolder("KagiSpy") then
            makefolder("KagiSpy")
        end
        if not isfolder("KagiSpy//Assets") then
            makefolder("KagiSpy//Assets")
        end
        if not isfile("KagiSpy//Settings.json") then
            writefile("KagiSpy//Settings.json",jsone(realconfigs))
        end
...
--- Prevents remote spam from causing lag (clears logs after `getgenv().KAGISPYCONFIG_MaxRemotes` or 500 remotes)
function clean()
    local max = getgenv().KAGISPYCONFIG_MaxRemotes
...
local customCursor = Create("ImageLabel",{Parent = KagiSpy3,Visible = false,Size = UDim2.fromOffset(200, 200),ZIndex = 1e9,BackgroundTransparency = 1,Image = "",Parent = KagiSpy3})
function mouseEntered()
    local con = connections["KAGISPY_CURSOR"]
    if con then
        con:Disconnect()
        connections["KAGISPY_CURSOR"] = nil
    end
    connections["KAGISPY_CURSOR"] = RunService.RenderStepped:Connect(function()
        ...
        if mouseInGui and getgenv().SimpleSpyExecuted then
            ...
            connections["KAGISPY_CURSOR"]:Disconnect()
        end
    end)
end
...
function backgroundUserInput(input)
    ...
    if input.UserInputType == Enum.UserInputType.MouseButton1 and inResizeRange then
        ...
        if not connections["KAGISPY_RESIZE"] then
            connections["KAGISPY_RESIZE"] = RunService.RenderStepped:Connect(function()
                local newPos = UserInputService:GetMouseLocation()
                if newPos ~= lastPos then
        ...
        table.insert(connections, UserInputService.InputEnded:Connect(function(inputE)
            if input == inputE then
                if connections["KAGISPY_RESIZE"] then
                    connections["KAGISPY_RESIZE"]:Disconnect()
                    connections["KAGISPY_RESIZE"] = nil
                end
            end
        end))
...
function newButton(name, description, onClick)
    ...
    local Text = Create("TextLabel",{Text = name,Name = "Text",Parent = FunctionTemplate,BackgroundColor3 = Color3.new(1, 1, 1),BackgroundTransparency = 1,Position = UDim2.new(0, 19, 0, 10),Size = UDim2.new(0, 69, 0, 18),ZIndex = 2,Font = Enum.Font.SourceSans,TextColor3 = Color3.fromRGB(253, 116, 255),TextSize = 14,TextStrokeColor3 = Color3.new(0.145098, 0.141176, 0.14902),TextXAlignment = Enum.TextXAlignment.Left})
...
function newRemote(type, data)
    ...
    local ColorBar = Create("Frame",{Name = "ColorBar",Parent = RemoteTemplate,BackgroundColor3 = (type == "event" and Color3.fromRGB(255, 242, 0)) or Color3.fromRGB(253, 116, 255),BorderSizePixel = 0,Position = UDim2.new(0, 0, 0, 1),Size = UDim2.new(0, 7, 0, 18),ZIndex = 2})
    local Text = Create("TextLabel",{TextTruncate = Enum.TextTruncate.AtEnd,Name = "Text",Parent = RemoteTemplate,BackgroundColor3 = Color3.new(1, 1, 1),BackgroundTransparency = 1,Position = UDim2.new(0, 12, 0, 1),Size = UDim2.new(0, 105, 0, 18),ZIndex = 2,Font = Enum.Font.SourceSans,Text = remote.Name,TextColor3 = Color3.fromRGB(253, 116, 255),TextSize = 14,TextXAlignment = Enum.TextXAlignment.Left})
    ...
    local connect = Button.MouseButton1Click:Connect(function()
        ...
        if blocked then
            log.GenScript = "-- THIS REMOTE WAS PREVENTED FROM FIRING TO THE SERVER BY KAGISPY\n\n" .. log.GenScript
        end
...
function t2s(t, l, p, n, vtv, i, pt, path, tables, tI)
    ...
    for k, v in next, t do -- iterates over table
        ...
        if size > (getgenv().KagiSpyMaxTableSize or 1000) then
            s = s .. "\n" .. string.rep(" ", l) .. "-- MAXIMUM TABLE SIZE REACHED, CHANGE 'getgenv().KagiSpyMaxTableSize' TO ADJUST MAXIMUM SIZE "
            break
        end
...
function formatstr(s, indentation)
    ...
    return '"' .. handled .. '"' .. (reachedMax and " --[[ MAXIMUM STRING SIZE REACHED, CHANGE 'getgenv().KagiSpyMaxStringSize' TO ADJUST MAXIMUM SIZE ]]" or "")
end
...
function handlespecials(s, indentation)
    ...
    until char == "" or i > (getgenv().KagiSpyMaxStringSize or 10000)
    while not isFinished(coroutines) do
        RunService.Heartbeat:Wait()
    end
    clear(coroutines)
    if i > (getgenv().KagiSpyMaxStringSize or 10000) then
        s = string.sub(s, 0, getgenv().KagiSpyMaxStringSize or 10000)
        return s, true
    end
    return s, false
end
...
local function taskscheduler()
    ...
    if #scheduled > KAGISPYCONFIG_MaxRemotes + 100 then
...
local function shutdown()
    ...
    getgenv().SimpleSpyExecuted = false; getgenv().KagiSpyExecuted = false
end
...
if not getgenv().SimpleSpyExecuted then
    local succeeded,err = pcall(function()
        if not RunService:IsClient() then
            error("KagiSpy cannot run on the server!")
        end
        getgenv().SimpleSpyShutdown = shutdown; getgenv().KagiSpyShutdown = shutdown
        onToggleButtonClick()
        if not hookmetamethod then
            ErrorPrompt("KagiSpy V3 will not function to it's fullest capablity due to your executor not supporting hookmetamethod.",true)
        end
        ...
        getgenv().SimpleSpy = SimpleSpy; getgenv().KagiSpy = SimpleSpy
        ...
        connectResize()
        KagiSpy3.Enabled = true
        logthread(spawn(function()
            delay(1,onToggleButtonUnhover)
        end))
        schedulerconnect = RunService.Heartbeat:Connect(taskscheduler)
        bringBackOnResize()
        KagiSpy3.Parent = (gethui and gethui()) or (syn and syn.protect_gui and syn.protect_gui(KagiSpy3)) or CoreGui
        logthread(spawn(function()
    ...
    if succeeded then
        getgenv().SimpleSpyExecuted = true; getgenv().KagiSpyExecuted = true
    else
        shutdown()
...
function()
    ...
    if func then
        ...
        codebox:setRaw("-- Calling function info\n-- Generated by the KagiSpy V3 serializer\n\n"..selected.Function)
        TextLabel.Text = "Done! Function info generated by the KagiSpy V3 Serializer."
    else
...
if configs.supersecretdevtoggle then
    ...
    local SuperSecretFolder = Create("Folder",{Parent = KagiSpy3})
...
if table.find({
    ...
    QuickCapture.Parent = KagiSpy3
    QuickCapture.BackgroundColor3 = Color3.fromRGB(15, 15, 15)
    QuickCapture.BackgroundTransparency = 0.14
    QuickCapture.Position = UDim2.new(0.529, 0, 0, 0)
    QuickCapture.Size = UDim2.new(0, 32, 0, 33)
    QuickCapture.Font = Enum.Font.SourceSansBold
    QuickCapture.Text = "KagiSpy"
    QuickCapture.TextColor3 = Background.Visible and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(253, 116, 255)
